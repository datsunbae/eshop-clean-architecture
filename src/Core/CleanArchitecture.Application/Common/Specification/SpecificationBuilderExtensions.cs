...
public static class SpecificationBuilderExtensions
{
        ...
        string filterOperator)
    {
        ...
        return filterOperator switch
        {
            ...
            _ => throw new InternalServerException("Filter Operator is not valid."),
        };
    }
        ...
        Type propertyType)
    {
        ...
        if (propertyType.IsEnum)
        {
            ...
            if (!Enum.TryParse(propertyType, stringEnum, true, out object? valueparsed)) throw new InternalServerException(string.Format("Value {0} is not valid for {1}", value, field));
        ...
        if (propertyType == typeof(Guid))
        {
            ...
            if (!Guid.TryParse(stringGuid, out Guid valueparsed)) throw new InternalServerException(string.Format("Value {0} is not valid for {1}", value, field));
    ...
    {
        if (string.IsNullOrEmpty(filter.Field)) throw new InternalServerException("The field attribute is required when declaring a filter");
        if (string.IsNullOrEmpty(filter.Operator)) throw new InternalServerException("The Operator attribute is required when declaring a filter");
        return filter;
    }
